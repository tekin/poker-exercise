...
class PokerTest < Minitest::Test
  ...
  def test_highest_card_10
    ...
    game = Poker.new([high_of_10, high_of_8])
  ...
  def test_two_pair
    ...
    game = Poker.new([pair_of_4, pair_of_2])
    assert_equal [pair_of_4], game.best_hand
  end
  ...
  def test_another_two_double_pair
    aces_and_twos = %w(2S AH 2C AD JH)
    aces_and_threes = %w(AS 3H 3S 8D AC)
    game = Poker.new([aces_and_twos, aces_and_threes])
    assert_equal [aces_and_threes], game.best_hand
  end
...
class HandTest < Minitest::Test
  def test_hand_highest_rank
    hand = Hand.new(%w(9S 8S 7S 6S 5S))
    assert_equal '9', hand.highest_rank
  end

  def test_another_hand_highest_rank
    hand = Hand.new(%w(9S 8S QS 6S 5S))
    assert_equal 'Q', hand.highest_rank
  end

  def test_pairs
    hand = Hand.new(%w(9S 8S QS 8D QH))
    assert_equal [
      [Card.new('8S'), Card.new('8D')],
      [Card.new('QS'), Card.new('QH')],
    ], hand.pairs
  end

  def test_three
    hand = Hand.new(%w(8H 8S QS 8D QH))
    assert_equal [Card.new('8H'), Card.new('8S'), Card.new('8D')], hand.three
  end

  def test_normal_straight
    hand = Hand.new(%w(4D 6H 3S 2D 5C))
...
class CardTest < Minitest::Test
  ...
  def test_equality
    card_a = Card.new("10S")
    card_b = Card.new("10S")
    assert_equal card_a, card_b
  end

  def test_inequality
    card_a = Card.new("10S")
    card_b = Card.new("9S")
    refute_equal card_a, card_b
  end
end
